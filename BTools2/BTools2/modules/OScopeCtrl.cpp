...
BOOL COScopeCtrl::Create(DWORD dwStyle, const RECT& rect, 
                         CWindow* pParentWnd, UINT nID) 
{
  BOOL result ;
  static CString className = "AfxRegisterWndClass(CS_HREDRAW | CS_VREDRAW)" ;

  result =  ::Create Create(WS_EX_CLIENTEDGE | WS_EX_OVERLAPPEDWINDOW   ,// | WS_EX_STATICEDGE, 
...
BOOL COScopeCtrl::PreTranslateMessage(MSG* pMsg)
{
	pMsg;
	return FALSE;
}
...
void COScopeCtrl::InvalidateCtrl()
{
  ...
  HPEN oldPen ;
  CPen solidPen;
  solidPen.CreatePen(PS_SOLID, 0, m_crGridColor) ;
  ...
  if (m_dcGrid.m_hDC == NULL)
  {
    ...
    m_pbitmapOldGrid = m_dcGrid.SelectBitmap(m_bitmapGrid) ;
  ...
  if(m_bitmapGrid.IsNull())
	  return;
  m_bitmapGrid.GetSize(bmSize);
  ...
  // draw the plot rectangle
  oldPen = m_dcGrid.SelectPen(solidPen) ; 
  ...
  oldFont = m_dcGrid.SelectFont(axisFont) ;
 ...
 // m_dcGrid.SetTextAlign (TA_LEFT|TA_TOP) ;
  ...
  //m_dcGrid.Rectangle(textRect);
  m_dcGrid.DrawText(L"0", -1, textRect, DT_LEFT|DT_NOCLIP);
  ...
  //m_dcGrid.Rectangle(textRect);
  m_dcGrid.DrawText(strTemp, -1,  textRect, DT_RIGHT|DT_NOCLIP);
  ...
  //m_dcGrid.Rectangle(textRect);
#endif
  m_dcGrid.DrawText(m_strXUnitsString, -1, textRect, DT_CENTER|DT_NOCLIP);
  ...
  // SetTextAlign과 DrawText의 조합은 정상적으로 동작하지 않은 듯 보인다.
   m_dcGrid.DrawText(m_strYUnitsString, -1,  textRect, DT_BOTTOM | DT_NOCLIP);
   /*
  ...
  if (m_dcPlot.m_hDC == NULL)
  {
    m_dcPlot.CreateCompatibleDC(dc) ;
	m_bitmapPlot.CreateCompatibleBitmap(dc, m_nClientWidth, m_nClientHeight) ;
    m_pbitmapOldPlot = m_dcPlot.SelectBitmap(m_bitmapPlot) ;
  }

  // just reuse
  if(m_bitmapPlot.IsNull())
	  return;
  m_bitmapPlot.GetSize(bmSize);
  if( bmSize.cx != m_nClientWidth ||
	  bmSize.cy != m_nClientHeight)
  {
	  m_dcPlot.SelectBitmap(m_pbitmapOldPlot) ;
...
////////////////////////////////////////////////////////////////////////////
void COScopeCtrl::OnPaint(CDCHandle ) 
{
	...
	if (!memBitmap.IsNull())// && memBitmap.IsNull() != NULL)
	{
...
void COScopeCtrl::DrawPoint()
{
  ...
  if (m_dcPlot.m_hDC != NULL)
  {
    ...
    oldPen = m_dcPlot.SelectPen(m_penPlot) ;
...
/////////////////////////////////////////////////////////////////////////////
void COScopeCtrl::OnSize(UINT nType, CSize size) 
{
