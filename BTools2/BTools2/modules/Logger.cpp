...
CString CLogRecord::GetFormatedMessage(void)
{
	...
	if( m_level == LEVEL_INFO )
	{
		...
		wsprintf(tempBuffer, "%3d ", m_timeStamp.millitm);
		result += tempBuffer;
...
void CLogger::SetLoggerName(CString loggerName)
{
	m_loggerName = loggerName;
	/*
	for (int i = 0; i < MAX_LOG_HANDLERS_NUMBER; i++)
	...
	DWORD sourceLine, CString message)
{
	...
	for (int i = 0; i < m_logHandlers.GetCount(); i++)
	{
		if (m_logHandlers[i] != NULL)
		{
			if (m_logHandlers.ElementAt(i)->Publish(record) != TRUE)
			{
				ASSERT(FALSE);
			}
		}
...
void CLogger::AddHandler(CLogHandler* logHandler)
{
	...
	for (int i = 0; i < m_logHandlers.GetCount(); i++)
	{
		CLogHandler *handler = m_logHandlers.ElementAt(i);
...
CLogHandler* CLogger::GetHandlerAt(DWORD index)
{
	...
	*/
	return m_logHandlers.ElementAt(index);
}

CLogHandler* CLogger::RemoveHandlerAt(DWORD index)
{
	CLogHandler*result = GetHandlerAt(index);
	return result;
}
