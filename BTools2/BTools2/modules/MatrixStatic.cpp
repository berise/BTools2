...
/** @brief  그림 크기,색 지정하기
    ...
    size:크기 color:색깔 1:blue 2:red
    @return void
*/
void CMatrixStatic::SetSize(int size, int bitmap)
{
	if (size == SMALL)
	{
	...
	else
	{
		...
		m_iImagematrix = bitmap;//IDB_MATRIXLARGE;
	}
...
/** @brief  그림 움직이는속도및 방향 정하기
    @param

    speed:그리는 속도 dir:움직이는 방향 (LEFT,RIGHT,UP,DOWN)
    @return void
*/
...
void CMatrixStatic::OnPaint(CDCHandle cdc)
{
...
COLORREF SEGM_COLORS[3] = {
	...
	CBitmapHandle pOldBitmap = dcMemory.SelectBitmap(bmp);
	int charcount = 0;
	int linecount = 1;
	...
	dcMemory.SelectBitmap(pOldBitmap);
	dcMemory.DeleteDC();
	DeleteObject(bmp);
