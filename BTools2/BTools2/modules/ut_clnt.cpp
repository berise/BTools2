...
int CUT_WSClient::WaitForAccept(long secs){
	...
	// warning C4127: conditional expression is constant - winsock.h macro contains while(0)
#pragma warning ( push )
...
int CUT_WSClient::Send(LPCSTR data, int len)
{
	...
	int	rt = SocketSend(m_socket,(char *)data,len,0);
...
int CUT_WSClient::Receive(LPSTR data,int maxLen,int timeOut){
    ...
    return SocketRecv(m_socket,data,maxLen,0);
...
int CUT_WSClient::ReceiveBlob(LPBYTE data,int dataLen,int timeOut){
	...
	while(dataRemain > 0){
		...
		int received = SocketRecv(m_socket,(char *)position,dataRemain,0);
...
int CUT_WSClient::ReceiveLine(LPSTR data,int maxDataLen,int timeOut){
	...
	while(maxDataLen > 0) {
		...
		rt = SocketRecv(m_socket,(char *)tempBuf,maxDataLen, MSG_PEEK);
		//error checking
		if(rt < 1 ) {
			break;
		}
		...
		for(int x = 0; x < rt; x ++){
			if(tempBuf[x] == '\n'){
				rt = SocketRecv(m_socket,(char *)dataPtr,x+1,0);
				dataPtr[x+1] = 0;
				count += x + 1;
				delete [] tempBuf;
				return count;
		...
		rt = SocketRecv(m_socket,(char *)dataPtr, rt, 0);
		count += rt;
		dataPtr[rt] = 0;
...
int CUT_WSClient::Receive(CUT_DataSource & dest, OpenMsgType type, int timeOut, long lMaxToReceive) 
{
    ...
    do{
		...
		if(lMaxToReceive > 0) {
			nSize = min(sizeof(data), lMaxToReceive - bytesReceived);
			if(nSize == 0)
				break;
			}
...
int CUT_WSClient::AsyncGetNameFromAddress(LPCSTR address,LPSTR name,int maxLen){
	...
	HANDLE h = NULL;
#ifndef _WIN32_WCE
	h = WSAAsyncGetHostByAddr(m_hAsyncWnd,CUT_ASYNC_ATON_MSG,
...
int CUT_WSClient::AsyncGetAddressFromName(LPCSTR name,LPSTR address,int maxLen){
	...
	HANDLE h= NULL;
#ifndef _WIN32_WCE
	h= WSAAsyncGetHostByName(m_hAsyncWnd,CUT_ASYNC_NTOA_MSG,
        &name[position],
...
Return
    port number
****************************************************/
const int CUT_WSClient::GetRemotePort() const
{
...
Return
    port number
****************************************************/
const int CUT_WSClient::GetLocalPort() const
{
	return m_nLocalPort;
}
...
Return
    port number
****************************************************/
const int CUT_WSClient::GetAcceptPort() const
{
