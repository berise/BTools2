...
//=============================================================================
	...
	m_crBackground(GetSysColor(COLOR_STATIC)),  //COLOR_BTNFACE)),
...
int CXGroupBox::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	::AtlMessageBox(NULL, L"CXGroupBox::OnCreate");
...
//=============================================================================
void CXGroupBox::PostSubclassWindow() 
//=============================================================================
{
	...
	ModifyStyle(WS_BORDER, 0);//|SS_SUNKEN
	...
	LOGFONT lf;
	lf.lfHeight			= 13;//DRA::SCALEY(12);//DRA::HIDPIMulDiv(10, nLogPixelsY, 72);
		lf.lfWidth			= 0;
		lf.lfEscapement		= 0;
		lf.lfOrientation	= 0;
...
//=============================================================================
void CXGroupBox::OnPaint(CDCHandle dc) 
//=============================================================================
{
...
//=============================================================================
{
	...
	if (m_sizeText.cx == 0)
	{
		...
		CFontHandle pOldFont = pDC->SelectFont(m_font);
		if (!strText.IsEmpty())
			m_sizeText = pDC->GetTextExtent(strText, 1, NULL);
		if (pOldFont)
	...
	CBitmapHandle pOldBitmap = memDC.SelectBitmap(bmp);
...
//=============================================================================
{
	if (!rect.IsRectEmpty())
	{
		CRect r(rect);
		CPen pen;
		pen.CreatePen(PS_SOLID, 5, crBackground);
...
//=============================================================================
{
	...
	CPen penDark;
	penDark.CreatePen(PS_SOLID, 1, crDark);
	CPenHandle pOldPen = pDC->SelectPen(penDark);
		
	if (m_eBorderStyle == etched)
	{
		// border is 2 pixels, so deflate bottom and right sides by 1
		CPen penLight;
		penLight.CreatePen(PS_SOLID, 2, GetSysColor(COLOR_BTNHIGHLIGHT));	// 2 pixels wide
		pDC->SelectPen(penLight);
...
//=============================================================================
{
	...
	if (m_eBorderStyle == etched)
	{
		...
		pDC->SelectPen(penLight);
		pDC->MoveTo(rect.left, (rect.bottom-rect.top)/2+1);
		pDC->LineTo(rect.right, (rect.bottom-rect.top)/2+1);
	}
		...
		*/
		{
			...
			DrawEtchedLine(pDC, rect,  m_crBorder);
...
//=============================================================================
{
	...
	if (!strText.IsEmpty())
	{
		...
		UINT nFormat =  DT_CENTER | DT_NOCLIP; // DT_VCENTER | DT_NOCLIP;//DT_SINGLELINE;

		CRect rectText(rect);
		// get size of text so we can update rect
		CSize size = pDC->GetTextExtent(strText, 1, NULL);
		rectText.right = rectText.left + size.cx;

		//m_dcGrid.DrawText(strTemp, -1, textRect, DT_CENTER|DT_NOCLIP);
		pDC->DrawText(strText, -1, rectText, nFormat);
...
//=============================================================================
BOOL CXGroupBox::OnEraseBkgnd(CDCHandle /*pDC*/) 
//=============================================================================
{
...
//=============================================================================
{
	...
	if (pFont == 0)
	{
		...
		if (pFont == 0)
		{
			...
			if (hFont == 0)
				1;//hFont = (HFONT)::GetStockObject(ANSI_VAR_FONT);
			if (hFont)
				pFont.Attach(hFont);
		}
	}
...
//=============================================================================
{
	...
	if (nIconId)
	{
		...
		m_hIcon = (HICON) ::AtlLoadIcon(nIconId);
		ASSERT(m_hIcon);
	}
	else
	{
...
int CXGroupBox::EnableChildWindows(HWND hWnd, BOOL bEnable, BOOL bEnableParent)
{
	...
	CWindow hWndParent = ::GetParent(hWnd);
	if (hWndParent.IsWindow())
		hWndChild = hWndParent.GetWindow(GW_CHILD);
