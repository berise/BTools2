...
/*static*/ HPALETTE CDIBSectionLite::CreateHalftonePalette(CPalette& palette, int nNumColors)
{
	if(!palette.IsNull())
...
{ 
   ...
   // We will only be able to get a HDRAWDIB if dithering is enabled
    ...
    else
    {
        ...
        if (!m_Palette.IsNull()&& UsesPalette(pDC))
        {
        ...
        bResult = pDC->BitBlt(ptDest.x, ptDest.y, size.cx, size.cy, pMemDC->m_hDC, 
...
{ 
    ...
    else
    {
       ...
       //CPalette* pOldPalette = NULL;
		HPALETTE pOldPalette;
        if (m_Palette.m_hPalette && UsesPalette(pDC))
        {
            pOldPalette = pDC->SelectPalette(m_Palette, bForceBackground);
            pDC->RealizePalette();
        }
                                ...
                                )
{
    ...
    //TRY {
	  ...
	  //DCR 4/02/01  I've found PocketPCs need BI_BITFIELDS for 16 bit dibs.
        ...
        GetColorTableEntries(dc.m_hDC, hBitmap);
        ...
        if (!m_Palette.IsNull())//.GetSafeHandle())
        {
        ...
        CopyDC.BitBlt(0,0, bm.bmWidth, bm.bmHeight, memDC.m_hDC, 0,0, SRCCOPY);
        ...
        if (!m_Palette.IsNull())//GetSafeHandle())
        {
            memDC.SelectStockPalette(DEFAULT_PALETTE, FALSE);
            CopyDC.SelectStockPalette(DEFAULT_PALETTE, FALSE);
        }
#endif // DIBSECTION_NO_PALETTE
    //}
		/*
...
BOOL CDIBSectionLite::Load(LPCTSTR lpszFileName)
{
    ...
    LONG dwFileStart = file.GetPosition();
    ...
    file.Seek(dwFileStart + BmpFileHdr.bfOffBits,FILE_BEGIN);//  CFile::begin);
...
BOOL CDIBSectionLite::Save(LPCTSTR lpszFileName)
{
    ...
    CFile file;
    if (!file.Open(lpszFileName, GENERIC_WRITE))//###//CFile::modeWrite|CFile::modeCreate))
        return FALSE;
...
HPALETTE CDIBSectionLite::CreatePalette()
{
	if(!m_Palette.IsNull())
    m_Palette.DeleteObject();
    ...
    HBITMAP hOldBitmap = (HBITMAP) ::SelectObject(MemDC.m_hDC, m_hBitmap);
    if (!hOldBitmap)
    {
        delete [] pRGB;
        return CreateHalftonePalette(m_Palette, m_iColorTableSize);
...
#ifdef _WIN32_WCE
    int nColors = ::CEGetDIBColorTable(MemDC.m_hDC, 0, m_iColorTableSize, pRGB);
#else
...
BOOL CDIBSectionLite::ReleaseMemoryDC(BOOL bForceRelease /*=FALSE*/)
{
    ...
    if (m_hOldBitmap)
        ::SelectObject(m_MemDC.m_hDC, m_hOldBitmap);
    m_hOldBitmap = NULL;
...
void CDIBSectionLite::Create32Bit(CDC *pDC, CDIBSectionLite &dib, int iWidth, int iHeight)
{
	...
	{	
		...
		{
			COLORREF clr = tempDC.GetPixel(j,i);
			pLine[(i*iWidth)+j] = FixColorRef(clr);
		}
	}
	// Set the dc back to normal
	tempDC.SelectBitmap(pOldBitmap);
}

//
//
void CDIBSectionLite::Create32Bit(CDIBSectionLite &dib, int iWidth, int iHeight)
{
	CBitmap ourNewSizedBMP;
	CWindowDC dc(NULL);
	CDC tempDC;
	...
	ourNewSizedBMP.CreateCompatibleBitmap(tempDC,iWidth,iHeight);
