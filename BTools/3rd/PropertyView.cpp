 ...
 * HISTORY		: =============================================================
 * 
 * $Log: PropertyView.cpp,v $
 * Revision 1.13  2001/06/05 10:54:07  leo
 * Impemented more compatible construction
 *
 * Revision 1.12  2001/05/15 15:35:06  leo
 * Fixed some bad TRACE strings for UNICODE compile
 *
 * Revision 1.11  2001/04/18 08:51:18  leo
 * Fixed a potential crash in CPropertyView::OnSize
 *
 * Revision 1.10  2001/01/04 17:14:02  leo
 * Improved page name handling
 *
 * Revision 1.9  2000/12/20 14:02:56  leo
...
CPropertyViewPage::CPropertyViewPage (LPCTSTR lpszTemplateName, UINT nIDCaption /* = 0 */) : baseCPropertyViewPage (lpszTemplateName, NULL)
{
...
#ifndef WINCE
CPropertyViewPage::CPropertyViewPage (UINT nIDTemplate, UINT nIDCaption /* = 0 */) : baseCPropertyViewPage (nIDTemplate, NULL)
{
	ASSERT(nIDTemplate != 0);
	CommonConstruct(MAKEINTRESOURCE(nIDTemplate), nIDCaption);
}
#else
CPropertyViewPage::CPropertyViewPage (UINT nIDTemplate, UINT nIDCaption /* = 0 */, CWnd *pParent) : baseCPropertyViewPage (nIDTemplate, pParent)
{
	ASSERT(nIDTemplate != 0);
	CommonConstruct(MAKEINTRESOURCE(nIDTemplate), nIDCaption);
}
...
void CPropertyViewPage::Construct (UINT nIDTemplate, UINT nIDCaption /* = 0 */)
{
	ASSERT(nIDTemplate != 0);
	CommonConstruct(MAKEINTRESOURCE(nIDTemplate), nIDCaption);
}

void CPropertyViewPage::Construct (LPCTSTR lpszTemplateName, UINT nIDCaption /* = 0 */)
{
	ASSERT(AfxIsValidString(lpszTemplateName));
	CommonConstruct(lpszTemplateName, nIDCaption);
}

void CPropertyViewPage::CommonConstruct (LPCTSTR lpszTemplateName, UINT nIDCaption)
{
...
int CPropertyViewPage::OnCreate (LPCREATESTRUCT lpCreateStruct)
{
	...
	if ( m_nIDCaption ) {
		CString csBuffer;
		if ( csBuffer.LoadString (m_nIDCaption) ) {
			SetWindowText (csBuffer);
		}
	}
...
BOOL CPropertyViewPage::OnKillActive ()
{
	...
	if ( !UpdateData () ) {
		TRACE(_T("UpdateData failed during page deactivation\n"));
		return FALSE;
	}
	return TRUE;
}
 ...
 * THE VIEW CLASS
 *============================================================================*/
IMPLEMENT_DYNCREATE(CPropertyView, baseCPropertyView)
...
BOOL CPropertyView::SetPageTitle (int nPage, UINT nID)
{
	CString csBuffer; csBuffer.LoadString (nID);
	return SetPageTitle (nPage, csBuffer);
}

BOOL CPropertyView::SetPageTitle (int nPage, LPCTSTR pszTitle)
{
	ASSERT(m_hWnd);
	ASSERT(m_TabCtrl.m_hWnd);
	if ( !m_hWnd || !m_TabCtrl.m_hWnd ) {
	...
	return m_TabCtrl.SetItem (nPage, &tcItem);
}

BOOL CPropertyView::GetPageTitle (int nPage, LPTSTR lpszStringBuf, int nMaxCount) const
{
	ASSERT(m_hWnd);
	ASSERT(m_TabCtrl.m_hWnd);
	if ( !m_hWnd || !m_TabCtrl.m_hWnd ) {
	...
	tcItem.mask			= TCIF_TEXT;
	tcItem.pszText		= lpszStringBuf;
	tcItem.cchTextMax	= nMaxCount;
	return m_TabCtrl.GetItem (nPage, &tcItem);
}

BOOL CPropertyView::AddPage (CPropertyViewPage *pPage, UINT nID, int nImage)
{
	CString csBuffer; csBuffer.LoadString (nID);
	return AddPage (pPage, csBuffer, nImage);
}

BOOL CPropertyView::AddPage (CPropertyViewPage *pPage, LPCTSTR pszTitle, int nImage)
{
	ASSERT(m_hWnd);
	ASSERT(m_TabCtrl.m_hWnd);
	ASSERT(pPage);
...
BEGIN_MESSAGE_MAP(CPropertyView, baseCPropertyView)
...
BOOL CPropertyView::PreTranslateMessage(MSG* pMsg) 
{
	...
	if ( baseCPropertyView::PreTranslateMessage(pMsg) ) {
		return TRUE;
	}
...
{
	baseCPropertyView::AssertValid();
}

/*
void CPropertyView::Dump(CDumpContext& dc) const
{
	baseCPropertyView::Dump(dc);
}
*/
#endif //_DEBUG
...
{
	if ( baseCPropertyView::OnCreate(lpCreateStruct) == -1 ) {
		return -1;
	}
...
void CPropertyView::OnSize(UINT nType, int cx, int cy) 
{
	...
	if ( !pPage ) {
		...
		baseCPropertyView::OnSize(nType, cx, cy);
...
{
	baseCPropertyView::OnSetFocus(pOldWnd);
	CPropertyViewPage *pPage = GetActivePage ();
	if ( pPage ) {
		pPage->SetFocus ();
	}
