...
/// md5 hashing
void md5_str(const char *hash_string, char *md5_output)
{
	...
	MD5_CTX m_md5;
		...
		MD5Init(&m_md5);
		//MD5Update(&m_md5, (unsigned char*)hashString2.GetBuffer(), hashString.GetLength());//strlen((char*)hashString));
		MD5Update(&m_md5, (unsigned char *)hash_string, strlen(hash_string));
		...
		{
			char t[3];
			sprintf(t, "%02lx",  m_md5.digest[j] & 0xFFl);
			strcat(md5_output, t);
		}
