...
void CMatrixStatic::SetText(LPCTSTR lpszText)
{
	if (m_csStrText != lpszText)
	{
		...
		if (m_bImmediateUpdate)
			...
			Invalidate(FALSE);// Since we're doing blitting		Invalidate();
	}
}
...
void CMatrixStatic::SetSize(int size)
{
	...
	else
	{
		...
		//m_iImagematrix = IDB_MATRIXSMALL;//IDB_MATRIXLARGE;
	}
...
// TODO:Double buffering for flicker free animation(OnTimer);
...
/*	COLORMAP	ColorMap[4];;
	...
	*/
	...
	// I don't like to reimplement double buffering again and again.
	// Just use exist source to do this.
	...
	for (int ix = 0; ix < strlen; ix++)
	{
		...
		off_screen_dc.BitBlt(x, y, (m_icharwidth + m_ixspacing), (m_icharheight+m_iyspacing), &dcMemory, rc.left, rc.top, SRCCOPY);
