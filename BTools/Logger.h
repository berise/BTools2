...
#include <afxtempl.h>	// MFC collectable objects
#include <afxwin.h>
//#include <io.h>
//#include <fcntl.h>
//#include <sys/types.h>
...
// use CArray to get handler pointers
...
/*!
	@class 실제적인 로그 처리를 담당하는 클래스
	@brief 로그 핸들러는 콘솔, 파일, 윈도우, 소켓 등의 구체적인 처리를 담당한다.
...
/*!
	@class 로그 처리를 담당하는 클래스
	@brief 로거는 로그 핸들러와 함께 로그 처리를 수행한다.
...
private:
	...
	CArray<CLogHandler*, CLogHandler*> m_logHandlers;
	//CLogHandler*m_logHandlers[MAX_LOG_HANDLERS_NUMBER];
public:
	CLogger();
	CLogger(CString loggerName);
	void SetLoggerName(CString loggerName);
