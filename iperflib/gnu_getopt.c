 ...
 *
 * $Id: gnu_getopt.c,v 1.3 2002/10/07 20:45:34 kgibbs Exp $
 */
...
#define NONOPTION_P (argv[gnu_optind][0] != '-' || argv[gnu_optind][1] == '\0')
    ...
    if ( longopts != NULL
       ...
       or abbreviated matches.  */
        ...
        if ( ambig && !exact ) {
            if ( gnu_opterr )
                fprintf (winCEStderr, _("%s: option `%s' is ambiguous\n"),
        ...
        if ( !long_only || argv[gnu_optind][1] == '-'
            ...
            if ( gnu_opterr ) {
                if ( argv[gnu_optind][1] == '-' )
                    /* --option */
                    fprintf (winCEStderr, _("%s: unrecognized option `--%s'\n"),
                             argv[0], nextchar);
                else
                    /* +option or -option */
                    fprintf (winCEStderr, _("%s: unrecognized option `%c%s'\n"),
...

    {
        ...
        if ( temp == NULL || c == ':' ) {
            if ( gnu_opterr ) {
                if ( posixly_correct )
                    ...
                    fprintf (winCEStderr, _("%s: illegal option -- %c\n"),
                             argv[0], c);
                else
                    fprintf (winCEStderr, _("%s: invalid option -- %c\n"),
                             argv[0], c);
            }
            gnu_optopt = c;
            return '?';
        ...
        if ( temp[0] == 'W' && temp[1] == ';' ) {
            ...
            if ( *nextchar != '\0' ) {
                ...
                if ( gnu_opterr ) {
                    /* 1003.2 specifies the format of this message.  */
                    fprintf (winCEStderr, _("%s: option requires an argument -- %c\n"),
                             argv[0], c);
                }
                gnu_optopt = c;
                if ( optstring[0] == ':' )
            ...
            if ( ambig && !exact ) {
                if ( gnu_opterr )
                    fprintf (winCEStderr, _("%s: option `-W %s' is ambiguous\n"),
                             argv[0], argv[gnu_optind]);
                nextchar += strlen (nextchar);
                gnu_optind++;
                return '?';
...
while ( 1 ) {
    ...
    switch ( c ) {
        ...
        default:
            printf ("?? gnu_getopt returned character code 0%o ??\n", c);
    }
}
